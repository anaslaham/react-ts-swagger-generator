...
import { parseSchema, getRefType, formDataTypes } from "../core/schema";
import { checkAndRenameModelName } from "../core/modelNames";
import { inlineExtraFormatters } from "../core/typeFormatters";
import {
...
const methodAliases = {
  ...
  patch: (pathName, hasPathInserts) =>
    _.camelCase(`${pathName}_partial_update`),
...
const isSuccessResponseStatus = (status) =>
  ...
  (+status >= SUCCESS_RESPONSE_STATUS_RANGE[0] &&
    +status < SUCCESS_RESPONSE_STATUS_RANGE[1]);
...
const getErrorReturnType = (responses, parsedSchemas, operationId) =>
  _.uniq(
    findBadResponses(responses)
      .map((response) =>
        getTypeFromRequestInfo(response, parsedSchemas, operationId)
      )
...
const getRouteParams = (parameters, where) =>
  collect(parameters, (parameter) => {
    ...
    return (
      refTypeInfo &&
      refTypeInfo.rawTypeData.in === where &&
      refTypeInfo.rawTypeData
    );
  });
    ...
    return [
      ...
      ..._.map(requestsMap, (requestInfo, method) => {
        ...
        if (formDataObjectSchema) {
          bodyType = parseSchema(
            formDataObjectSchema,
            null,
            inlineExtraFormatters
          ).content;
        } else if (requestBody) {
          bodyType = getTypeFromRequestInfo(
            requestBody,
            parsedSchemas,
            operationId
          );
          ...
          if (
            formDataTypes.some((formDataType) =>
              _.includes(bodyType, `: ${formDataType}`)
            )
          ) {
        ...
        const insideRoutePathArgs = _.compact(
          _.split(route, "{").map((part) =>
            part.includes("}") ? part.split("}")[0] : null
          )
        );
        ...
        const pathArgs = insideRoutePathArgs.length
          ? _.map(pathParams, (param) => ({
              ...
              type: parseSchema(param.schema, null, inlineExtraFormatters)
                .content,
            }))
          : [];
        ...
        insideRoutePathArgs.forEach((routePathArg) => {
          ...
          if (
            !pathArgs.find(
              (pathArg) => pathArg && pathArg.name === routePathArg
            )
          ) {
            pathArgs.push({
              name: routePathArg,
        ...
        const specificArgs = {
          query: queryType
            ? {
                name: pathArgs.some((pathArg) => pathArg.name === "query")
                  ? "queryParams"
                  : "query",
                optional: parseSchema(queryObjectSchema, null)
          ...
          body: bodyType
            ? {
                name: bodyParamName,
                optional:
                  typeof requestBody.required === "undefined"
                    ? false
